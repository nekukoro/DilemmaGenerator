...
const templates = [
  ...
  },
  {
    ...
    text: "{{Creature1}}が「来週の平日のどこかで{{Action1}}をするが、いつやるかは当日まで予測できないようにする。」と言った。しかし、木曜日までに行われなかったら金曜日に行われると予測でき、同様に木曜、<span>水曜...</span>と逆算すると、{{Action1}}は不可能であるという結論が導かれる。この結論は正しいだろうか。"
  },
  {
    id: "survival",
    title: "臓器くじ",
  ...
  },
  {
    id: "grandfather",
    title: "親殺しのパラドクス",
    text: "{{Creature1}}はタイムマシンで過去に戻り、自分の先祖を{{Action1}}した。その結果、{{Creature1}}は存在しなくなるはずだが、存在しないなら{{Action1}}をすることも不可能になる。"
  },
  {
    id: "MontyHall",
    title: "モンティーホール問題",
...
function generateDilemma() {
  ...
  const generatedHTML = text.replace(placeholderRegex, (match, type, id) => {
...
function dilemmaUI() {
  ...
  const charSpeed = 100; // 元々40
  const highlightPause = 500; // 元々400
  ...
  // すべての文字が出揃ったらボタンを有効化し、グリッチを開始する
  setTimeout(() => {
...
function startRandomGlitch(cloneElement) {
  stopRandomGlitchTimerOnly(); // タイマーだけリセット
...
function execGlitch(clone) {
  try {
    if (!clone || !document.body.contains(clone)) {
      console.error("【ERROR】クローンがDOM上に存在しません");
      return;
    }
...
if (relotteryBtn) {
  relotteryBtn.addEventListener('click', generateDilemma);
}